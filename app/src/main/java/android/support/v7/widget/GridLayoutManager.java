...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    View findReferenceChild(int start, int end, int itemCount) {
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount) {
                int span = this.mSpanSizeLookup.getCachedSpanIndex(position, this.mSpanCount);
                if (span != 0) {
                    continue;
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LinearLayoutManager.LayoutState layoutState, LinearLayoutManager.LayoutChunkResult result) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (this.mOrientation == 1) {
                measureChildWithDecorationsAndMargin(view2, spec, getMainDirSpec(lp.height));
            } else {
                measureChildWithDecorationsAndMargin(view2, getMainDirSpec(lp.width), spec);
            }
        ...
        for (int i2 = 0; i2 < count; i2++) {
            ...
            if (this.mOrientationHelper.getDecoratedMeasurement(view3) != maxSize) {
                ...
                if (this.mOrientation == 1) {
                    measureChildWithDecorationsAndMargin(view3, spec2, maxMeasureSpec);
                } else {
                    measureChildWithDecorationsAndMargin(view3, maxMeasureSpec, spec2);
                }
            }
        }
