    ...
    public static class HitBuilder<T extends HitBuilder> {
        ...
        public T addImpression(Product product, String impressionList) {
            if (product == null) {
                ...
                if (!this.zzLJ.containsKey(impressionList)) {
                    this.zzLJ.put(impressionList, new ArrayList());
                }
                this.zzLJ.get(impressionList).add(product);
            }
            return this;
        }
        ...
        public T addPromotion(Promotion promotion) {
            if (promotion == null) {
                ...
                this.zzLK.add(promotion);
            }
            return this;
        }

        public Map<String, String> build() {
            HashMap map = new HashMap(this.zzLH);
            if (this.zzLI != null) {
                map.putAll(this.zzLI.build());
            }
            Iterator<Promotion> it = this.zzLK.iterator();
            int i = 1;
            while (it.hasNext()) {
                map.putAll(it.next().zzaX(zzc.zzV(i)));
                i++;
            }
            Iterator<Product> it2 = this.zzLL.iterator();
            int i2 = 1;
            while (it2.hasNext()) {
                map.putAll(it2.next().zzaX(zzc.zzT(i2)));
                i2++;
            }
            int i3 = 1;
            for (Map.Entry<String, List<Product>> entry : this.zzLJ.entrySet()) {
                List<Product> value = entry.getValue();
                String strZzY = zzc.zzY(i3);
                Iterator<Product> it3 = value.iterator();
                int i4 = 1;
                while (it3.hasNext()) {
                    map.putAll(it3.next().zzaX(strZzY + zzc.zzX(i4)));
                    i4++;
                }
                if (!TextUtils.isEmpty(entry.getKey())) {
                    map.put(strZzY + "nm", entry.getKey());
                }
                i3++;
            }
            return map;
        ...
        protected String get(String paramName) {
            return this.zzLH.get(paramName);
        }

        public final T set(String paramName, String paramValue) {
            if (paramName != null) {
                this.zzLH.put(paramName, paramValue);
            } else {
        ...
        public final T setAll(Map<String, String> params) {
            if (params != null) {
                this.zzLH.putAll(new HashMap(params));
            }
            return this;
        }

        public T setCampaignParamsFromUrl(String utmParams) {
            String strZzbr = zzam.zzbr(utmParams);
            if (!TextUtils.isEmpty(strZzbr)) {
                Map<String, String> mapZzbp = zzam.zzbp(strZzbr);
                set("&cc", mapZzbp.get("utm_content"));
                set("&cm", mapZzbp.get("utm_medium"));
                set("&cn", mapZzbp.get("utm_campaign"));
                set("&cs", mapZzbp.get("utm_source"));
                set("&ck", mapZzbp.get("utm_term"));
                set("&ci", mapZzbp.get("utm_id"));
                set("&anid", mapZzbp.get("anid"));
                set("&gclid", mapZzbp.get("gclid"));
                set("&dclid", mapZzbp.get("dclid"));
                set("&aclid", mapZzbp.get("aclid"));
                set("&gmob_t", mapZzbp.get("gmob_t"));
            }
            return this;
        }

        public T setCustomDimension(int index, String dimension) {
            set(zzc.zzP(index), dimension);
            return this;
        }

        public T setCustomMetric(int index, float metric) {
            set(zzc.zzR(index), Float.toString(metric));
            return this;
        }
        ...
        public T setNewSession() {
            set("&sc", Constants.DEFAULT_START_PAGE_NAME);
            return this;
        }
